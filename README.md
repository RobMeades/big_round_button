...
## Everything Else
Everything else should be printed in natural ASA (for UV-safety, natural ASA for improved reflectivity), fastest speed aside from `big_round_button_usb_c_hole_cap.stl` (which is small enough to need highest detail, e.g. 0.1&nbsp;mm layer height), 15% in fill, no supports required except for `big_round_button_back.stl` and `big_round_button_usb_c_hole_cap.stl`, which are relatively complex shapes and so should have supports everywhere.  As with all of the larger/flatter parts I got the room up to 45&nbsp;C with a space heater (my printer is inside a large wooden box but has no direct heating), got my slicer program to add a draft excluder (where room was available), minimised the fan and reduced the speed to 40&nbsp;mm/s max.

## Optional `big_round_button_sign.stl`
If you want to print an optional sign, create some text in Blender (a sample is provided) and do the Blender Knife Project thang to cut it into the blank sign (extruding the letters you have cut upwards by 1&nbsp;mm).  Export the resulting object to `.stl` at a Blender scale factor of 1000 to get millimetres.  It is then cool to set your printer's slicer program to print the top layer (thickness-wise) in black ASA and the lower part in natural ASA, saving a painting step.  Otherwise print settings are the same as for everything else, see above.